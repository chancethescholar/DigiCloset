...
class Closet:
    ...
    def image_to_binary(self, path):     
        # remove background to get a clearer image for binary image   
        self.remove_background(path)
    ...
    def get_overlap(self, clothing_binary, template_binary):
        ...
        # if ratio is over 0.68, then it is a match
    ...
    def get_convex_hull(self, threshold, binary2):
        #find contours
        ...
        try:
            ...
            return self.get_convex_hull(binary2, binary2)
        ...
        if defects is None:
            ...
            return self.get_convex_hull(threshold, threshold)
    ...
    def count_defects(self, img, thresh, defects, approx):      
        # l = no. of defects
        l = 0
    ...
    def get_defects(self, img, clothing_binary, clothing_binary2, count):
        approx, defects, clothing_contour, clothing_binary = self.get_convex_hull(clothing_binary, clothing_binary2)
        clothing_num_defects = self.count_defects(img, clothing_binary, defects, approx)

        #print(clothing_num_defects)
        if count == 0 and clothing_num_defects == 0:
            return self.get_defects(img, clothing_binary2, clothing_binary2, count+1)

        return clothing_num_defects, clothing_binary

    def contour_match(self, clothing_num_defects, template_num_defects):
        #print("defects", clothing_num_defects, template_num_defects)
        return clothing_num_defects == template_num_defects

    def match_template(self, clothing_num_defects, clothing_binary, template_num_defects, template_binary):
        # match clothing item to top template
        # by finding overlap of binary images
        overlap, ratio = self.get_overlap(clothing_binary, template_binary)

        # and checking if number of contours match
        contour = self.contour_match(clothing_num_defects, template_num_defects)
    ...
    def get_colors(self, img, binary):
        #cv2.imshow("binary", binary)
        #cv2.waitKey(0)
        #cv2.destroyAllWindows()
    ...
    def add_clothing(self, clothing_path, next_id):
        clothing_img, no_bg, clothing_binary, clothing_binary2 = self.image_to_binary(clothing_path)
        ...
        # find the defects in contour for each image
        clothing_num_defects, clothing_binary = self.get_defects(clothing_img_dup, clothing_binary, clothing_binary2, 0)

        shoe_num_defects, shoe_binary = self.get_defects(templates["shoe"]["image"], templates["shoe"]["binary"], templates["shoe"]["binary2"], 0)
        top_num_defects, top_binary = self.get_defects(templates["top"]["image"], templates["top"]["binary"], templates["top"]["binary2"], 0)
        sweater_num_defects, sweater_binary = self.get_defects(templates["sweater"]["image"], templates["sweater"]["binary"], templates["sweater"]["binary2"], 0)
        shorts_num_defects, shorts_binary = self.get_defects(templates["shorts"]["image"], templates["shorts"]["binary"], templates["shorts"]["binary2"], 0)
        pants_num_defects, pants_binary = self.get_defects(templates["pants"]["image"], templates["pants"]["binary"], templates["pants"]["binary2"], 0)
        skirt_num_defects, skirt_binary = self.get_defects(templates["skirt"]["image"], templates["skirt"]["binary"], templates["skirt"]["binary2"], 0)
        dress_num_defects, dress_binary = self.get_defects(templates["dress"]["image"], templates["dress"]["binary"], templates["dress"]["binary2"], 0)
        jacket_num_defects, jacket_binary = self.get_defects(templates["jacket"]["image"], templates["jacket"]["binary"], templates["jacket"]["binary2"], 0)

        # use dictionary to keep track of overlap ratios with each clothing type
        all_overlap_ratios = {}

        # check if defects and overlap match with template
        shoe_is_match, shoe_ratio = self.match_template(clothing_num_defects, clothing_binary, shoe_num_defects, shoe_binary)
        all_overlap_ratios["Shoe"] = shoe_ratio

        top_is_match, top_ratio = self.match_template(clothing_num_defects, clothing_binary, top_num_defects, top_binary)
        all_overlap_ratios["Top"] = top_ratio

        sweater_is_match, sweater_ratio = self.match_template(clothing_num_defects, clothing_binary, sweater_num_defects, sweater_binary)
        all_overlap_ratios["Sweater"] = sweater_ratio

        shorts_is_match, shorts_ratio = self.match_template(clothing_num_defects, clothing_binary, shorts_num_defects, shorts_binary)
        all_overlap_ratios["Shorts"] = shorts_ratio

        pants_is_match, pants_ratio = self.match_template(clothing_num_defects, clothing_binary, pants_num_defects, pants_binary)
        all_overlap_ratios["Pants"] = pants_ratio

        skirt_is_match, skirt_ratio = self.match_template(clothing_num_defects, clothing_binary, skirt_num_defects, skirt_binary)
        all_overlap_ratios["Skirt"] = skirt_ratio

        dress_is_match, dress_ratio = self.match_template(clothing_num_defects, clothing_binary, dress_num_defects, dress_binary)
        all_overlap_ratios["Dress"] = dress_ratio

        jacket_is_match, jacket_ratio = self.match_template(clothing_num_defects, clothing_binary, jacket_num_defects, jacket_binary)
        all_overlap_ratios["Jacket"] = jacket_ratio

        # use dictionary to keep track of overlap ratios for clothing type that came out to be a match
        ...
        # if no match has been found, choose the clothing type with the largest overlap ratio
        if len(match_overlap_ratios) == 0:
            clothing_type = max(all_overlap_ratios.items(), key=operator.itemgetter(1))[0]

        # if more than one match has been found, choose the largest overlap ratio from the matched clothing types only
        else:
            clothing_type = max(match_overlap_ratios.items(), key=operator.itemgetter(1))[0]

        # get clothing colors
        colors = self.get_colors(clothing_img, clothing_binary)

        #print(clothing_type)
        #print(colors)

        # save uploaded image to frontend folder to be used in React app
        path = os.path.abspath('../frontend/public/Clothing')
        file_name = str(next_id) + '.jpeg'

        cv2.imwrite(os.path.join(path , file_name), clothing_img)

        # return added clothing with type, colors, and the correct image path to be added to database
        clothing = {
            "type": clothing_type,
            "colors": colors,
            "image_path": 'Clothing/' + file_name,
        }
